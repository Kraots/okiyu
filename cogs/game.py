...
class _Game(commands.Cog, name='Game'):
    ...
    async def game_shop(self, ctx: Context):
        ...
        **NOTE:** This command can only be used in <#913330644875104306>
        ...
        em.add_field(
            f'Epic Box — 750,000 {self.coin_emoji}',
            'This box will get you characters that have a rarity of ✮✮✮✮(4)',
            inline=False
        )
        em.add_field(
            f'Legendary Box — 1,500,000 {self.coin_emoji}',
    ...
    async def game_blackjack(self, ctx: Context, *, amount: str):
        ...
        data = await self.get_user(ctx.author.id)
        if amount == 'all':
            amount = 10000 if data.coins >= 10000 else data.coins
        else:
            amount = utils.format_amount(amount)
            try:
                amount = int(amount)
        ...
        if amount < 200:
            ...
            em = utils.fail_embed(f'You can\'t bet more than **10,000** {self.coin_emoji}')
            return await ctx.reply(embed=em)
